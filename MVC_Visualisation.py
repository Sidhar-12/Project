...
def MVC_Visualisation(list_edges,random_vertex_cover,n):
    ...
    for vertex in G.nodes():
        if vertex in random_vertex_cover:
            colors.append('red')
        else:
            colors.append('blue') 
...
def index_solution(sat_list,dic,min_no_one,list_edges,n):
    ...
    print('All Minimum vertex covers are',vertex_cover) #list of all vertex covers
    random_vertex_cover = sorted(random.choice(vertex_cover) ) 
    #print('One of the Vertx Cover',random_vertex_cover) #Random vertex cover for given graph
    k=MVC_Visualisation(list_edges,random_vertex_cover,n)
    return k    

def optimal_sat_sol(list_edges,n,T=None):
    constraint=[]
    for i in (list_edges):
        var1=f'x_{i[0]}'
        var2=f'x_{i[1]}'
...
if 'y'in list_edges:
    ...
    print(optimal_sat_sol(list_edges,n_V)) 

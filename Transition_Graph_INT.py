...
def Visualise_Max_Match(Remng_B_edge,Attack_edge,partition1,partition2,B_edge,s1,s2,list_edges,edges_cmbntn_s1s2,file_name1,file_name2): #B_edge:Bipartite edges
    ...
    A=[]
    for i in Attack_edge[0]:
        A.append(int(i[1:]))
    l=len(max_matching)
    if l!=len(s1):
        return f'The attacking edge {A} can not defeated by {file_name1} to get {file_name2} in one time step'
...
def final_Bipartite(B_edge,Attack_Edge,max_match,s1,s2,list_edges,edges_cmbntn_s1s2,file_name1,file_name2):
    ...
    plt.show()
    f=Transition_Graph(B_edge,Attack_Edge,max_match,s1,s2,list_edges,edges_cmbntn_s1s2,file_name1,file_name2)
    return f

def Transition_Graph(B_edge,Attack_Edgee,max_match,s1,s2,list_edges,edges_cmbntn_s1s2,file_name1,file_name2):
    #print(s1,'and',s2)
    #print('edges',list_edges)

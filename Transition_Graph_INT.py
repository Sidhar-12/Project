...
def Z3_solver(list_edges,n,T,file_name):
    ...
    l_G=[]#list of all text files,depand upon the values of No of True 
    if s.check()==sat:
        solution=(s.model())
        solution=(solution) 
        count=1
        ...
        print(f'All Text files are inside the {subfolder_name}')
        ...
        file_name = "G{}.txt".format(count)
        l_G.append(file_name)
            ...
            if s.check()==sat: 
                ...
                for i in solution:
                    if solution[i]==1:
                        l.append(int(str(i)[1:]))
                file_name = "G{}.txt".format(count)
                l_G.append(file_name) #Append all Text file one by one
        ...
        return (f'Vertex Cover is not Possisble for {T} no of True Values.')
    print("List of All Text Files:", l_G) 

    'Enter Folder and file name for further work'
    ...
    if os.path.exists(folder_path) and os.path.isdir(folder_path):
        ...
        v=Vetex_Cover_Use_File(G1,G2,solution,folder_path,file_name1,file_name2,file_name)
        return v
    
def Vetex_Cover_Use_File(G1,G2,solution,folder_path,file_name1,file_name2,file_name):
    '''G1=open("G1.txt", "r")
    G2=open("G1.txt", "r")
    vertex=[]
    edges_list=[]
    ...
    m=MVC_Visualisation(Graph_edges_list,s1_vertex_cover,s2_vertex_cover,file_name1,file_name2,file_name) 
    return m

def MVC_Visualisation(list_edges,s1,s2,file_name1,file_name2,file_name):#n=no of vertices
...
def Maximum_Matching_Edges(B_edge,Attack_Edge,partition1,partition2,s1,s2,list_edges,edges_cmbntn_s1s2,file_name1,file_name2): #B_edge:All Bipartite edges
    #print('Bipartite edges',B_edge)
    Remng_B_edge=[] #Removing attack edge from bipartite, then remaing edge 
    for i,x in enumerate(B_edge):
        if x[0]==Attack_Edge[0] or x[1]==Attack_Edge[1]:
            pass
        else:
...
def final_Bipartite(B_edge,Attack_Edge,max_match,s1,s2,list_edges,edges_cmbntn_s1s2,file_name1,file_name2):
    ...
    #print('1',bipartite_E)    
    Attack_E=[]
    Attack_E.append((f'x{Attack_Edge[0]}',f'y{Attack_Edge[1]}')) 
    #print('2',Attack_E)  
    #print('3',max_match)
...
def final_Output(B_edge,Attack_Edge,max_match,s1,s2,list_edges,edges_cmbntn_s1s2,file_name1,file_name2):
    #print(s1,'and',s2)
    #print('edges',list_edges)
    ...
    if Attack_Edge not in edges_cmbntn_s1s2:
        ...
        nx.draw(G1, with_labels=True, node_color=colors_s1, node_size=500,ax=axes[0])
        ...
        #nx.draw(G1, with_labels=True, node_color='skyblue', font_color='black')
        nx.draw(G2, with_labels=True, node_color=colors_s2, node_size=500,ax=axes[1])
        plt.plot([], [],'o', label='Vertex Cover', color='red') 
        ...
        nx.draw_networkx_nodes(G, pos, node_size=300,ax=axes[1])
        nx.draw_networkx_edges(G, pos, edgelist=list_edges, width=1, edge_color='b', arrows=False,ax=axes[1])
        nx.draw_networkx_edges(G, pos, edgelist=Max_M, width=1, edge_color='r', arrows=True, arrowstyle='-|>', arrowsize=6,ax=axes[1])
        nx.draw_networkx_labels(G, pos, font_size=10, font_family='sans-serif',ax=axes[1])
        ...
        #nx.draw(G1, with_labels=True, node_color='skyblue', font_color='black')
        nx.draw(G2, with_labels=True, node_color=colors_s2, node_size=500,ax=axes[2])
...
#print(list_edges,n)         
T=int(input('No of Guards:'))
print(Z3_solver(list_edges,n,T,file_name))